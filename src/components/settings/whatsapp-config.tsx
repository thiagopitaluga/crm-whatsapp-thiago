...
import { QrWhatsAppConnector } from './qr-whatsapp-connector';
...
type ConnectionMethod = 'official' | 'qr';
...
export function WhatsAppConfig() {
  ...
  const [connectionMethod, setConnectionMethod] =
    useState<ConnectionMethod>('official');
  ...
  useEffect(() => {
    ...
    const saved = window.localStorage.getItem(
      `whatsapp-connection-method:${accountId}`
    );
    if (saved === 'official' || saved === 'qr') {
      setConnectionMethod(saved);
    }
  }, [accountId]);
  ...
  async function handleReset() {
    if (
      !confirm(
        'A configuração atual do WhatsApp será excluída para que você possa informá-la novamente. Continuar?'
      )
    ) {
      return;
    }
  ...
  return (
    <section className="animate-in fade-in-50 duration-200">
      ...
      <Tabs
        ...
        onValueChange={(value) =>
          selectConnectionMethod(value as ConnectionMethod)
        }
        className="gap-5"
      >
        <TabsList className="border-border bg-muted/40 h-auto w-full justify-start gap-1 rounded-xl border p-1 sm:w-fit">
          <TabsTrigger value="official" className="min-h-10 px-4 sm:min-w-56">
            <Cloud className="size-4" />
            {t('methodOfficial')}
          </TabsTrigger>
          <TabsTrigger value="qr" className="min-h-10 px-4 sm:min-w-56">
                    ...
                    <AccordionItem className="border-border">
                      ...
                      <AccordionContent className="text-muted-foreground">
                        <ol className="list-inside list-decimal space-y-1 text-sm">
                          ...
                          <li
                            dangerouslySetInnerHTML={{
                              __html: t.raw('step3_2'),
                            }}
                          />
                          <li
                            dangerouslySetInnerHTML={{
                              __html: t.raw('step3_3'),
                            }}
                          />
                          <li
                            dangerouslySetInnerHTML={{
                              __html: t.raw('step3_4'),
                            }}
                          />
                        </ol>
                      </AccordionContent>
                    ...
                    <AccordionItem className="border-border">
                      ...
                      <AccordionContent className="text-muted-foreground">
                        <ol className="list-inside list-decimal space-y-1 text-sm">
                          ...
                          <li
                            dangerouslySetInnerHTML={{
                              __html: t.raw('step4_3'),
                            }}
                          />
                          <li
                            dangerouslySetInnerHTML={{
                              __html: t.raw('step4_4'),
                            }}
                          />
                          <li>{t('step4_5')}</li>
                        </ol>
        ...
        <TabsContent value="qr">
          <QrWhatsAppConnector disabled={!canEditSettings} />
        </TabsContent>
      </Tabs>
    </section>
  );
