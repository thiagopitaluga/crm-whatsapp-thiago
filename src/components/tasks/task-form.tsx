...
export function TaskForm({
  ...
  useEffect(() => {
    ...
    setContactId(
      task?.contact_id ?? defaultDeal?.contact_id ?? defaultContactId ?? ''
    );
    setAssignedTo(
      task?.assigned_to ?? defaultDeal?.assigned_to ?? profile?.id ?? ''
    );
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return;
      setContacts(
        (contactsResult.data ?? []) as Pick<Contact, 'id' | 'name' | 'phone'>[]
      );
      setMembers(
        (membersResult.data ?? []) as Pick<
          Profile,
          'id' | 'full_name' | 'email'
        >[]
      );
