...
| `automations:manage` | Create, edit, activate, and delete automations |
...
List automations or create a disabled automation draft. Scope:
`automations:manage`. Creation requires `name` and `trigger_type`; it
accepts optional `description`, `trigger_config`, and the builder's nested
`steps` tree. New automations are always created with `is_active: false`.

### `GET` / `PATCH` / `DELETE /api/v1/automations/{id}`

Read, update, or delete an automation in the API key's account. Scope:
`automations:manage`. `GET` includes the full nested step tree. Setting
`is_active: true` on `PATCH` validates the trigger and steps before the
automation can run.
